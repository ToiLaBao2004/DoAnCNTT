...
{
    // Định nghĩa một struct để đại diện cho doanh thu theo ngày
    public struct RevenueByDate
    {
        public string Date { get; set; } // Thuộc tính Ngay để lưu trữ ngày
        public int TotalAmount { get; set; } // Thuộc tính TongTien để lưu trữ tổng số tiền
    }

    // Defining the DBThongKe class inheriting from the DAL class
    public class DBThongKe
    {
        private DateTime startDate; // Biến startDate để lưu trữ ngày bắt đầu
        private DateTime endDate; // Biến endDate để lưu trữ ngày kết thúc
        private int numberDays; // Biến numberDays để lưu trữ số ngày

        // Khai báo các thuộc tính
        public int NumCustommers { get; private set; } // Số lượng khách hàng
        public int NumSuppliers { get; private set; } // Số lượng nhà cung cấp
        public int NumProduct { get; private set; } // Số lượng sản phẩm
        public List<KeyValuePair<string, int>> TopProductsList { get; private set; } // Danh sách các sản phẩm hàng đầu
        public List<KeyValuePair<string, int>> UnderstocksList { get; private set; } // Danh sách các sản phẩm sắp hết hàng
        public List<RevenueByDate> GrossRevenueList { get; private set; } // Danh sách doanh thu gộp theo ngày
        public int NumOrder { get; set; } // Số lượng đơn hàng
        ...
        private void GetNumberItems()
        {
            using (var context = new QLCuaHang())
            {
                ...
                NumCustommers = context.Customers.Count(); // Đếm số lượng khách hàng
                NumSuppliers = context.Suppliers.Count(); // Đếm số lượng nhà cung cấp
                NumProduct = context.Products.Count(); // Đếm số lượng sản phẩm
                NumOrder = context.Orders
                    .Where(o => o.OrderDate >= startDate && o.OrderDate <= endDate)
                    .Count(); // Đếm số lượng đơn hàng trong khoảng thời gian từ startDate đến endDate
            }
        }
